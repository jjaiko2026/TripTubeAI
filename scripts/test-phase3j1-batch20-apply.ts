/**
 * PHASE 3-J-1 — 20건 상세정보 배치 반영 (INSERT/UPDATE 있음, 숙박 없음).
 *
 * 실행: dotenv -e .env.local -- tsx scripts/test-phase3j1-batch20-apply.ts
 *
 * Phase 3-G-2/3-I와의 핵심 차이(사용자 승인 지시): 이전 Phase들은 place 1건이라도 API
 * 실패/매핑 오류가 나면 스크립트 전체를 하드 스톱했다. 이번엔 "20건은 최대 처리 대상이지
 * 반드시 전부 성공해야 하는 건수가 아니다" — place 단위로 실패를 격리하고 건별로 집계만
 * 하며, 나머지 후보는 계속 처리한다. detailCommon2가 실패하면 그 place는 places UPDATE만
 * 건너뛰고, detailIntro2가 실패하면 그 place는 place_tourism_details INSERT만 건너뛴다 —
 * 두 호출/두 쓰기가 서로 독립이다(Phase 3-G-2/3-I의 원자적 트랜잭션 결합과 다름, 이번엔
 * 의도적으로 분리).
 *
 * 여전히 place당 정확히 1건 매칭만 UPDATE 대상으로 삼는다(UNIQUE(external_source,
 * external_content_id) 덕분에 구조적으로 0/1건만 가능) — "선정된 20개 contentId 외
 * 다른 place UPDATE 금지"를 SQL WHERE 절로 강제한다.
 */
export {};
...
// PHASE 3-J-1 PREVIEW에서 확정한 20건 그대로(라운드로빈 선정, place_tourism_details 없음 확인됨).
const TEST_TARGETS = [
  { regionCode: "KR-SEOUL-CITY", regionNameKo: "서울", expectedAreaCode: "1", expectedSigunguCode: null as string | null, contentTypeId: "12", contentId: "2453179", name: "고촌이종근기념관" },
  { regionCode: "KR-SEOUL-CITY", regionNameKo: "서울", expectedAreaCode: "1", expectedSigunguCode: null as string | null, contentTypeId: "14", contentId: "130232", name: "건국대학교 상허기념도서관" },
  { regionCode: "KR-JEJU-JEJUSI", regionNameKo: "제주시", expectedAreaCode: "39", expectedSigunguCode: "4", contentTypeId: "12", contentId: "129073", name: "구엄어촌체험마을" },
  { regionCode: "KR-JEJU-JEJUSI", regionNameKo: "제주시", expectedAreaCode: "39", expectedSigunguCode: "4", contentTypeId: "14", contentId: "2949573", name: "제주문학관" },
  { regionCode: "KR-JEJU-JEJUSI", regionNameKo: "제주시", expectedAreaCode: "39", expectedSigunguCode: "4", contentTypeId: "39", contentId: "2850913", name: "가는곶 세화" },
  { regionCode: "KR-JEJU-SEOGWIPO", regionNameKo: "서귀포시", expectedAreaCode: "39", expectedSigunguCode: "3", contentTypeId: "12", contentId: "1887866", name: "갑선이오름" },
  { regionCode: "KR-JEJU-SEOGWIPO", regionNameKo: "서귀포시", expectedAreaCode: "39", expectedSigunguCode: "3", contentTypeId: "14", contentId: "2751836", name: "바이나흐튼 크리스마스박물관" },
  { regionCode: "KR-SEOUL-CITY", regionNameKo: "서울", expectedAreaCode: "1", expectedSigunguCode: null as string | null, contentTypeId: "12", contentId: "264570", name: "강남" },
  { regionCode: "KR-SEOUL-CITY", regionNameKo: "서울", expectedAreaCode: "1", expectedSigunguCode: null as string | null, contentTypeId: "14", contentId: "129831", name: "고려대학교 박물관" },
  { regionCode: "KR-JEJU-JEJUSI", regionNameKo: "제주시", expectedAreaCode: "39", expectedSigunguCode: "4", contentTypeId: "12", contentId: "1918646", name: "거슨새미오름" },
  { regionCode: "KR-JEJU-JEJUSI", regionNameKo: "제주시", expectedAreaCode: "39", expectedSigunguCode: "4", contentTypeId: "14", contentId: "2905116", name: "제주과학탐구체험관" },
  { regionCode: "KR-JEJU-JEJUSI", regionNameKo: "제주시", expectedAreaCode: "39", expectedSigunguCode: "4", contentTypeId: "39", contentId: "1867998", name: "고불락" },
  { regionCode: "KR-JEJU-SEOGWIPO", regionNameKo: "서귀포시", expectedAreaCode: "39", expectedSigunguCode: "3", contentTypeId: "12", contentId: "2516416", name: "낙타트레킹" },
  { regionCode: "KR-JEJU-SEOGWIPO", regionNameKo: "서귀포시", expectedAreaCode: "39", expectedSigunguCode: "3", contentTypeId: "14", contentId: "2606690", name: "왈종미술관" },
  { regionCode: "KR-SEOUL-CITY", regionNameKo: "서울", expectedAreaCode: "1", expectedSigunguCode: null as string | null, contentTypeId: "12", contentId: "1826438", name: "경찰혼" },
  { regionCode: "KR-SEOUL-CITY", regionNameKo: "서울", expectedAreaCode: "1", expectedSigunguCode: null as string | null, contentTypeId: "14", contentId: "130587", name: "갤러리조선" },
  { regionCode: "KR-JEJU-JEJUSI", regionNameKo: "제주시", expectedAreaCode: "39", expectedSigunguCode: "4", contentTypeId: "12", contentId: "2791433", name: "고내포구" },
  { regionCode: "KR-JEJU-JEJUSI", regionNameKo: "제주시", expectedAreaCode: "39", expectedSigunguCode: "4", contentTypeId: "14", contentId: "129895", name: "제주 탑동해변공연장" },
  { regionCode: "KR-JEJU-JEJUSI", regionNameKo: "제주시", expectedAreaCode: "39", expectedSigunguCode: "4", contentTypeId: "39", contentId: "2870515", name: "경성수산" },
  { regionCode: "KR-JEJU-SEOGWIPO", regionNameKo: "서귀포시", expectedAreaCode: "39", expectedSigunguCode: "3", contentTypeId: "12", contentId: "600584", name: "금호리조트 제주아쿠아나" },
] as const;
...
async function main() {
  ...
  for (const t of TEST_TARGETS) {
    ...
    if (hasExistingDetail) {
      ...
      console.log(`  주의: 기존 place_tourism_details 발견 — 이 place는 INSERT하지 않음(스킵)`);
    }

    // ── detailCommon2 (독립 시도, 실패해도 detailIntro2는 계속 진행) ──
    apiCallCount++;
    ...
    if (!common.ok) {
      ...
      commonFailed++;
      failureLog.push(`contentId=${t.contentId}: detailCommon2 응답 불일치(${common.items.length}건, contentid=${common.items[0]?.contentid})`);
    ...
    // ── places UPDATE (detailCommon2 성공 + 지역 매핑 통과 시에만) ──
    if (commonOk) {
      ...
      if (!regionCheck.ok) {
        ...
        try {
          ...
          if (updateRows.length === 1 && (updateRows[0] as any).id === place.id) {
            ...
            console.log(`  places UPDATE 실패: 대상 불일치(${updateRows.length}건)`);
          }
        } catch (e) {
          updateFailed++;
    ...
    // ── place_tourism_details INSERT (detailIntro2 성공 + 기존 detail 없을 때만) ──
    if (introOk && !hasExistingDetail) {
      ...
      try {
        ...
        if (insertRows.length === 1) {
          ...
          insertFailed++;
          failureLog.push(`contentId=${t.contentId}: INSERT 결과 이상(${insertRows.length}건)`);
  ...
  console.log("\n=== PHASE 3-J-1 COMPLETE ===");
  console.log(`API 호출: 계획 ${TEST_TARGETS.length * 2} / 실제 ${apiCallCount}`);
