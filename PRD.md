...
- **AI 생성 비용**: 여행 1건당 AI 호출 1회 + 실제 검색 최대 16회(유튜브/블로그 각 최대 8회)로, 트래픽이 늘면 YouTube 일일 쿼터·AI Gateway 크레딧 소모를 모니터링해야 함
- **`/plan/example` 캐시**: 1시간 단위로 캐시되어 비로그인 방문자에게는 쿼터 절약이 되지만, 최초 캐시 미스 시 응답이 느릴 수 있음(관측된 첫 생성 약 27초)
