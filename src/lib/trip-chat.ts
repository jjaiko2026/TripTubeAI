...
export function buildTripChatSystemPrompt(currentDraft: TripRequest): string {
  return [
    ...
    `다음 지명은 이름만으로 어느 지역인지 특정할 수 없거나(광주/고성) 나라 단위라 범위가 너무 넓습니다: ` +
      `${describeAmbiguousDestinations()}. 사용자가 이런 이름을 말하면 임의로 추측하지 말고 어느 지역/도시인지 ` +
      "반드시 되물어서 확인한 뒤 구체적인 지역명으로 확정해서 destination에 전달하세요 (예: '광주' → 되물은 뒤 " +
      "'광주광역시' 또는 '경기 광주시'로 확정, '베트남' → 되물은 뒤 '다낭'으로 확정). 위에 나온 도시들은 예시일 " +
      "뿐이니, 사용자가 목록에 없는 다른 구체적인 도시(예: '나트랑', '붕따우')를 직접 말하면 그 이름 그대로 " +
      "destination에 반영하세요 — 목록에 있는 도시로 바꾸도록 유도하지 마세요.",
